...
#include "Arduino.h"
// Incluido en fingerprint_esp32.h
...
RegisterRoutine registerRoutine = RegisterRoutine();
FingerprintSensorRoutine fingerPrintSensorRoutine = FingerprintSensorRoutine(&SerialPort);
...
*/
static const u_char RECV_OK[] = {0x01, 'R', 'E', 'C', 'V', 'O', 'K', 0x04, 0x00};
static const u_char RECV_BAD[] = {0x01, 'R', 'E', 'C', 'V', 'B', 'A', 'D', 0x04, 0x00};
static const u_char SET_MODE_REGISTER[] = {0x01, 'R', 'E', 'G', 0x04, 0x00};
static const u_char SET_MODE_AUTH[] = {0x01, 'A', 'U', 'T', 'H', 0x04, 0x00};
static const u_char DISCONNECT[] = {0x01, 'F', 'I', 'N', 0x04, 0x00};
...
void handle_client(int clientSock)
{
    ...
    while (1)
    {
        ...
        while (buf[i] != EOT)
        {
            ...
            if (i == 0 && buf[i] != SOH)
                break; // ignore packet without SOH
        ...
        // Enviar datos desde el sensor
        else
        {

            switch (registerRoutine.getRegisterRoutine())
            {
            case true:
            ...
            default:
                ...
                // Recibir huella desde el sensor --> empaquetar datos --> enviar al cliente --> esperar respuesta

                uint8_t *data = fingerPrintSensorRoutine.getActualFingerData();

                DataPacket packet = DataPacket(data);

                ssize_t bytes_write = write(clientSock, packet.data(), packet.dataSize());

                if (bytes_write == -1)
                {
                    // Error on writing
                    perror("writing fingerprint");
                    send_err(clientSock);
                    continue;
                }
